...
ok('v16-6-stable-full-system' in index,'index missing V16.6 release marker')
...
ok('v15-tech.css' not in index and 'v15-tech.css' not in exam and 'v15-tech.css' not in sw,'stale v15-tech.css dependency still referenced')
# Every relative JS/CSS file referenced by production HTML/SW must exist.
refs=set(re.findall(r'["\'](\./[^"\']+?\.(?:js|css))(?:\?[^"\']*)?["\']',index+'\n'+exam+'\n'+sw))
for ref in refs:
    ok((SITE/ref[2:]).is_file(),f'missing local dependency {ref}')
...
ok('doc-full-nr-v16-6-stable-20260914' in sw,'service worker cache marker is not V16.6')
platform=(SITE/'v16-platform.js').read_text('utf-8')
app=(SITE/'app.js').read_text('utf-8')
ok('verifyOtp' not in platform,'OTP verify flow still present in production platform')
ok('set_phone_otp_enforcement' not in platform,'OTP enforcement switch still present in production platform')
ok('ยืนยันเบอร์โทรศัพท์ด้วย OTP' not in platform,'OTP phone screen still present')
...
ok('nickname' in app,'nickname field missing from registration/profile')
ok('พ่อคุณเป็นฝรั่งหรอ' in app,'English-input alert missing')
ok('display_name' in platform,'nickname display_name mapping missing')
ok('v164-filter-grid' in platform,'advanced search filters missing')
ok('v164-profile-kpis' in platform,'profile analytics UI missing')
ok('birth_date' in app,'birth date missing')
ok('join_subject_with_code' in platform,'join-code enrollment missing')
ok('v165-course-gallery' in platform,'course catalog missing')
ok('v165-room-code' in platform,'admin join-code UI missing')
ok('nav-card-menu' in app,'card navigation missing')
ok('data-edit-user' in app and 'editUserDialog' in app,'Admin user edit flow missing')
ok('admin-create-user' in app and 'birth_date' in app and 'display_name' in app,'Admin student create form incomplete')
ok('เลขประจำตัวนักศึกษา / Login' in app,'student login sync edit UI missing')
ok('nav-card-label' in platform,'card-nav DOM preservation marker missing')
ok('V16.6-STABLE-FULL-SYSTEM' in platform,'V16.6 platform marker missing')
...
for marker in ['v162-grade-report-table','exportGradeSummaryExcel','loadSubjectWorkDetailData','v162-web-work-details','ภาระงาน(','รวม(100)']:
    ok(marker in platform,f'V16.2 grade report marker missing: {marker}')
...
print('release=V16.6 STABLE; files=',len(required),'manifest_icons=',len(icons))
