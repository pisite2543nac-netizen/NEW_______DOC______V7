...
with sync_playwright() as pw:
    ...
    page.evaluate("""(profile)=>{
      ...
      globalThis.__createClient=function(){
        ...
        return {from:(t)=>chain(t),rpc:async(name,args)=>{if(name==='server_now')return {data:new Date().toISOString(),error:null};if(name==='admin_system_health_v17')return {data:{backend_ok:true,active_subjects:11,standard_templates:198,critical_rpcs:15},error:null};if(name==='my_leader_classrooms')return {data:[],error:null};return {data:null,error:null};},realtime:{setAuth:async()=>true},channel:()=>({on(){return this},subscribe(){return this}}),removeChannel:()=>true,storage:{from:()=>({createSignedUrl:async()=>({data:{signedUrl:'about:blank'},error:null})})}};
      };
