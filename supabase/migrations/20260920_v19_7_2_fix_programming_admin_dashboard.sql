-- DOC-FULL-NR V19.7.2
-- Fix PL/pgSQL output-column ambiguity in Admin Programming Activity Dashboard.
begin;
drop function if exists public.admin_programming_activity_dashboard_v197(uuid);
create function public.admin_programming_activity_dashboard_v197(p_subject_id uuid)
returns table(
...
begin
 ...
 select stt.id,stt.student_code,stt.full_name,stt.class_name,
   stt.completed,stt.html_stage,stt.python_stage,stt.attempts,
   round(stt.bw,1),round(stt.aa,1),round((stt.rj->>'rating')::numeric,1),stt.rj->>'tier',stt.tok,
   stt.offc,round(stt.offs,2),stt.offsub,stt.foc
