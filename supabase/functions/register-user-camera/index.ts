...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const studentCode=String(body.student_code||body.username||'').trim(),username=studentCode,password=String(body.password||''),fullName=String(body.full_name||'').trim();
    ...
    const phone=normalizePhone(body.phone),photoSource=String(body.profile_photo_source||''),photo=decodeJpeg(body.profile_photo_jpeg),contact=String(body.email||'').trim().toLowerCase()||null;
    if(!/^\d{1,15}$/.test(studentCode)||password.length<8||!fullName||!LEVELS.has(gradeLevel)||!ROOMS.has(roomLabel)||!DEPARTMENTS.has(department)||!MAJORS.has(major))return json({error:'INVALID_REGISTRATION_DATA'},400);
    ...
    const {data:created,error}=await svc.auth.admin.createUser({email:authEmail,password,email_confirm:true,user_metadata:{full_name:fullName,username,student_code:studentCode,profile_photo_source:'camera_live'}});if(error)return json({error:error.message},400);
    ...
    const {error:pe}=await svc.from('profiles').update({full_name:fullName,display_name:fullName,username,role:'user',student_code:studentCode,class_name:className,grade_level:gradeLevel,room_label:roomLabel,department,major,phone,phone_verified_at:null,contact_email:contact,avatar_path:avatarPath,active:false,approval_status:'pending',approval_requested_at:now,academic_status:'studying'}).eq('id',id);
    ...
    await svc.from('audit_logs').insert({actor_id:null,action:'REGISTER_USER',entity_type:'profile',entity_id:id,metadata:{self_registration:true,approval_status:'pending',student_code:studentCode,grade_level:gradeLevel,room_label:roomLabel,department,major,phone_provided:!!phone,phone_verified:false,profile_photo:'camera_live',avatar_path:avatarPath,classroom_id:roomId,academic_year:academicYear,semester}});
    return json({ok:true,username:studentCode,approval_status:'pending',avatar_path:avatarPath,phone,phone_verified:false,classroom_id:roomId,academic_year:academicYear,semester});
