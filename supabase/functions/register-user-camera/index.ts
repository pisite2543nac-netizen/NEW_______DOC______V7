...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const studentCode=String(body.student_code||body.username||'').trim(),username=studentCode,password=String(body.password||''),fullName=String(body.full_name||'').trim(),nickname=String(body.nickname||'').trim();
    ...
    const {error:pe}=await svc.from('profiles').update({full_name:fullName,display_name:nickname,username,role:'user',student_code:studentCode,class_name:className,grade_level:gradeLevel,room_label:roomLabel,department,major,phone,phone_verified_at:null,contact_email:null,avatar_path:avatarPath,active:false,approval_status:'pending',approval_requested_at:now,academic_status:'studying'}).eq('id',id);
    ...
    await svc.from('audit_logs').insert({actor_id:null,action:'REGISTER_USER',entity_type:'profile',entity_id:id,metadata:{self_registration:true,approval_status:'pending',student_code:studentCode,nickname,grade_level:gradeLevel,room_label:roomLabel,department,major,phone_provided:!!phone,phone_verified:false,profile_photo:'camera_live',avatar_path:avatarPath,classroom_id:roomId,academic_year:academicYear,semester}});
    return json({ok:true,username:studentCode,nickname,approval_status:'pending',avatar_path:avatarPath,phone,phone_verified:false,classroom_id:roomId,academic_year:academicYear,semester});
