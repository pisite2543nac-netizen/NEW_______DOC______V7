...
function analyticalQuestions(domain,topic,concepts=[]){
  const c=concepts.length?concepts:[topic];
  const x={
    law:[
      `จากสถานการณ์ที่เกี่ยวข้องกับ “${topic}” ข้อใดเป็น “ข้อเท็จจริง” และข้อใดเป็น “ข้อสรุป/ความคิดเห็น” เพราะเหตุใด`,
      `ถ้าข้อมูลของนายจ้างและลูกจ้างขัดแย้งกัน ควรตรวจเอกสารหรือเงื่อนไขใดก่อนจึงจะวิเคราะห์ “${topic}” ได้อย่างเป็นธรรม`,
      `เปรียบเทียบทางเลือก 2 แนวทางในการจัดการเรื่อง “${topic}” แล้วอธิบายผลต่อสิทธิ หน้าที่ และความเสี่ยงของแต่ละฝ่าย`,
      `หากเปลี่ยนเงื่อนไขสำคัญหนึ่งข้อในกรณีศึกษา ผลการวิเคราะห์ควรเปลี่ยนหรือไม่ เพราะเหตุใด`,
      `อะไรคือหลักฐานขั้นต่ำที่ต้องมีก่อนตัดสินใจในเรื่อง “${topic}” และเพราะเหตุใดหลักฐานนั้นจึงสำคัญ`,
      `แยกแยะว่าแนวทางใดเป็นการป้องกันปัญหาล่วงหน้า และแนวทางใดเป็นการแก้ปัญหาหลังเกิดเหตุ`,
      `หากมีข้อยกเว้นของหลักเกณฑ์เกิดขึ้น จะตรวจอย่างไรว่าข้อยกเว้นนั้นใช้กับกรณีนี้จริง`,
      `สรุปเหตุผลของคุณโดยเชื่อมอย่างน้อย 2 แนวคิด ได้แก่ ${c.slice(0,2).join(' และ ')}`
    ],
    safety:[
      `จากสถานการณ์ “${topic}” ระบุอันตรายกับความเสี่ยงแยกจากกัน แล้วอธิบายว่าทั้งสองต่างกันอย่างไร`,
      `ถ้ามีมาตรการควบคุมได้หลายแบบ ควรเลือกมาตรการใดก่อนและเพราะเหตุใด`,
      `เปรียบเทียบการควบคุมที่ต้นเหตุกับการใช้ PPE ว่าแบบใดลดความเสี่ยงได้ยั่งยืนกว่าในกรณีนี้`,
      `หากเกิดเหตุซ้ำแม้มีมาตรการแล้ว ควรสงสัย Root cause จุดใดก่อน`,
      `ข้อมูลใดบ้างที่ต้องเก็บเพื่อพิสูจน์ว่าความเสี่ยงลดลงจริงหลังแก้ไข`,
      `แยกแยะ “สภาพไม่ปลอดภัย” กับ “การกระทำไม่ปลอดภัย” ในกรณีตัวอย่างของหน่วยนี้`,
      `หากต้องจัดลำดับแก้ไข 3 ปัญหา จะใช้เกณฑ์โอกาสเกิดและความรุนแรงอย่างไร`,
      `เสนอแนวทางหนึ่งที่ลดความเสี่ยงโดยไม่พึ่งพาความจำหรือพฤติกรรมของบุคคลเพียงอย่างเดียว`
    ],
    network:[
      `ถ้าอุปกรณ์เชื่อมต่อเครือข่ายได้แต่ใช้บริการไม่ได้ จะใช้หลักฐานใดแยกว่าเป็นปัญหา Link, IP, Gateway, DNS หรือ Application`,
      `เปรียบเทียบอาการของการตั้งค่า IP ผิดกับ DNS ผิด ว่ามีจุดสังเกตต่างกันอย่างไร`,
      `ถ้าผู้ใช้บางคนใช้งานได้แต่บางคนไม่ได้ ขอบเขตปัญหาน่าจะอยู่ตรงไหน และควรตรวจอะไรต่อ`,
      `เหตุใดการเปลี่ยนค่าหลายจุดพร้อมกันจึงทำให้การวิเคราะห์ Root cause ยากขึ้น`,
      `เลือกเครื่องมือหรือคำสั่งตรวจสอบ 2 อย่าง แล้วอธิบายว่าหลักฐานจากแต่ละอย่างช่วยยืนยันสมมติฐานใด`,
      `หากแก้ปัญหาแล้วควรทดสอบปลายทางใดบ้างเพื่อแยกแยะว่า “หายจริง” กับ “หายเฉพาะจุด”`,
      `เปรียบเทียบผลกระทบของการตั้งค่าผิดในชั้นกายภาพกับชั้นบริการ`,
      `ออกแบบลำดับ Troubleshooting ที่สั้นที่สุดแต่ยังตรวจสอบย้อนกลับได้สำหรับ “${topic}”`
    ],
    ui:[
      `จากหน้าจอที่เกี่ยวข้องกับ “${topic}” แยกปัญหา “สวยแต่ใช้ยาก” กับ “ใช้ง่ายแต่ข้อมูลไม่ครบ” แล้วเสนอวิธีทดสอบ`,
      `เปรียบเทียบ Design ทางเลือก 2 แบบโดยใช้เกณฑ์ User Goal, เวลาในการทำงาน และข้อผิดพลาด แทนความชอบส่วนตัว`,
      `ถ้าผู้ใช้ทำขั้นตอนผิดซ้ำ จุดใดใน Information Architecture หรือ Feedback ควรถูกตรวจเป็นอันดับแรก`,
      `องค์ประกอบใดควรเด่นที่สุดบนหน้าจอ และหลักฐานใดสนับสนุนการตัดสินใจนั้น`,
      `แยกแยะปัญหาที่เกิดจาก Visual Hierarchy กับปัญหาที่เกิดจาก User Flow`,
      `ถ้าผู้ใช้บนมือถือทำงานสำเร็จน้อยกว่า Desktop ควรเก็บข้อมูลอะไรเพื่อหาสาเหตุ`,
      `เสนอการปรับหนึ่งจุดที่ช่วย Accessibility โดยไม่ทำให้ผู้ใช้ทั่วไปใช้งานยากขึ้น`,
      `อธิบายว่าผล Usability Test แบบใดเพียงพอที่จะเปลี่ยน Design และแบบใดยังไม่เพียงพอ`
    ],
    frontend:[
      `ถ้าหน้าเว็บทำงานบน Desktop แต่ผิดบน Mobile จะตรวจ Structure, CSS, Script และ State อย่างไรเพื่อแยกสาเหตุ`,
      `เปรียบเทียบปัญหาที่เกิดจาก Layout กับปัญหาที่เกิดจาก JavaScript โดยใช้หลักฐานจาก Browser DevTools`,
      `ถ้าแก้ CSS แล้วหน้าหนึ่งดีขึ้นแต่อีกหน้าพัง แสดงว่ามี Coupling แบบใดและควรลดอย่างไร`,
      `เหตุใดการรองรับ Loading, Empty, Error และ Offline จึงถือเป็นส่วนหนึ่งของ “ความถูกต้อง” ของ Front-End`,
      `เลือกหนึ่งปัญหา Accessibility แล้วอธิบายว่าจะตรวจด้วย Keyboard และ Semantic structure อย่างไร`,
      `ถ้า Fetch API ล้มเหลวเป็นบางครั้ง ควรแยก Network error, Server error และ Client state อย่างไร`,
      `เปรียบเทียบการแก้เฉพาะอาการกับการแก้ Root cause ในหัวข้อ “${topic}”`,
      `ออกแบบ Regression test ขั้นต่ำที่จะยืนยันว่าการแก้ไขไม่ทำส่วนเดิมพัง`
    ],
    mobile:[
      `ถ้าแอปทำงานได้เมื่อออนไลน์แต่ผิดเมื่อ Offline ควรแยกปัญหาที่ Storage, Sync, Retry หรือ UI state อย่างไร`,
      `เปรียบเทียบกรณีผู้ใช้ “ปฏิเสธ Permission” กับ “อุปกรณ์ไม่มี Feature” ว่า Flow สำรองควรต่างกันอย่างไร`,
      `เมื่อแอปถูกพักแล้วกลับมาใช้งาน ข้อมูลใดควรถูกตรวจใหม่ก่อนแสดงผล`,
      `เหตุใดการทดสอบบน Emulator อย่างเดียวจึงอาจไม่พอสำหรับ “${topic}”`,
      `ถ้าต้องเลือกระหว่างประหยัดแบตเตอรี่กับ Sync ข้อมูลทันที จะใช้เงื่อนไขใดตัดสิน`,
      `แยกแยะข้อมูลที่ควรเก็บ Local กับข้อมูลที่ควรอ่านจาก Server ทุกครั้ง`,
      `เสนอวิธีจัดการ Error ที่ช่วยให้ผู้ใช้แก้สถานการณ์ได้เองโดยไม่สูญเสียข้อมูล`,
      `ออกแบบกรณีทดสอบหนึ่งชุดที่รวม Network หลุด, Permission เปลี่ยน และ App lifecycle`
    ],
    programming:[
      `ให้ติดตาม Input → Process → Output ของ “${topic}” แล้วระบุจุดที่ผลลัพธ์อาจผิดได้อย่างน้อย 2 จุด`,
      `เปรียบเทียบ Test case ปกติกับ Boundary case ว่าแต่ละแบบค้นหาข้อผิดพลาดชนิดใดได้ดีกว่า`,
      `ถ้าโปรแกรมให้ผลถูกกับข้อมูลส่วนใหญ่แต่ผิดบางค่า ควรตรวจเงื่อนไข ชนิดข้อมูล หรือลูปอย่างไรเพื่อแยกสาเหตุ`,
      `แยกแยะ Syntax error, Runtime error และ Logic error พร้อมยกตัวอย่างที่สัมพันธ์กับ “${topic}”`,
      `ถ้ามีวิธีเขียนได้ 2 แบบ ให้เปรียบเทียบด้านความอ่านง่าย ความถูกต้อง และการทดสอบ`,
      `เหตุใดการแก้ค่าจนตัวอย่างเดียวผ่านจึงยังไม่ถือว่าแก้ปัญหาเสร็จ`,
      `เลือกข้อมูลทดสอบที่มีโอกาสเปิดเผย Bug มากที่สุด 3 ค่า พร้อมเหตุผล`,
      `อธิบาย Root cause ของกรณีตัวอย่างโดยเชื่อมอย่างน้อย 2 แนวคิด ได้แก่ ${c.slice(0,2).join(' และ ')}`
    ],
    data:[
      `จากข้อมูลนำเข้า แยก “ข้อมูลผิดรูปแบบ” “ข้อมูลซ้ำ” และ “Mapping ผิด” ว่าควรตรวจพบในขั้นใด`,
      `เปรียบเทียบการ Reject record กับการแก้ค่าอัตโนมัติ ว่าแต่ละแบบเหมาะกับสถานการณ์ใด`,
      `ถ้าจำนวน Record ถูกต้องแต่รายงานปลายทางผิด จะตรวจ Transformation หรือ Storage อย่างไร`,
      `หลักฐานใดช่วยยืนยันว่า Import สำเร็จจริงมากกว่าการดูเพียงว่าไม่มี Error`,
      `แยกแยะ Validation ก่อนนำเข้ากับ Quality check หลังนำเข้า พร้อมยกตัวอย่าง`,
      `หากนำเข้าไฟล์เดิมซ้ำ ระบบควรตัดสินอย่างไรว่าเป็นข้อมูลเดิมหรือข้อมูลใหม่`,
      `ถ้าต้อง Rollback การนำเข้า จะต้องเก็บข้อมูลหรือ Log อะไรไว้ล่วงหน้า`,
      `ออกแบบกฎหนึ่งข้อที่ลดความผิดพลาดของ “${topic}” โดยยังไม่ปฏิเสธข้อมูลที่ถูกต้องมากเกินไป`
    ],
    service:[
      `จาก Ticket เรื่อง “${topic}” แยก “อาการ” กับ “สาเหตุ” แล้วอธิบายว่าทำไมสองอย่างนี้ไม่ควรถูกสรุปเป็นเรื่องเดียวกัน`,
      `หากมี Ticket หลายรายการพร้อมกัน ควรจัดลำดับด้วยผลกระทบและความเร่งด่วนอย่างไร`,
      `เปรียบเทียบการแก้ชั่วคราวกับการแก้ Root cause ว่าเมื่อใดควรใช้แต่ละแบบ`,
      `ข้อมูลใดควรเก็บก่อนเปลี่ยนค่าเพื่อให้ย้อนกลับได้หากการแก้ไขไม่สำเร็จ`,
      `ถ้าผู้ใช้บอกว่า “ยังช้า” หลังแก้แล้ว ควรใช้หลักฐานอะไรยืนยันหรือหักล้าง`,
      `แยกแยะ Preventive Maintenance กับ Corrective Maintenance ในบริบทของหน่วยนี้`,
      `เหตุใดการยืนยันผลกับผู้ใช้จึงเป็นส่วนหนึ่งของการตรวจคุณภาพ ไม่ใช่เพียงมารยาทบริการ`,
      `ออกแบบข้อความสรุปปิด Ticket ที่ทำให้ช่างคนถัดไปตรวจสอบย้อนหลังได้`
    ],
    general:[
      `จากสถานการณ์ของ “${topic}” ระบุข้อเท็จจริง สมมติฐาน และข้อมูลที่ยังขาดให้แยกจากกัน`,
      `เปรียบเทียบทางเลือก 2 วิธีแล้วอธิบายข้อดี ข้อจำกัด และเงื่อนไขที่ทำให้แต่ละวิธีเหมาะสม`,
      `หากผลลัพธ์ไม่เป็นไปตามคาด ควรตรวจ Root cause จุดใดก่อนและเพราะเหตุใด`,
      `หลักฐานใดเพียงพอที่จะสรุปว่าวิธีที่เลือกใช้ได้ผลจริง`,
      `แยกแยะการแก้ “อาการ” กับการแก้ “สาเหตุ” ในหัวข้อนี้`,
      `ถ้าเปลี่ยนเงื่อนไขสำคัญหนึ่งข้อ ผลการตัดสินใจควรเปลี่ยนอย่างไร`,
      `เสนอวิธีตรวจผลที่ช่วยลดการสรุปจากความรู้สึกหรือความเคยชิน`,
      `เชื่อมอย่างน้อย 2 แนวคิด ได้แก่ ${c.slice(0,2).join(' และ ')} เพื่ออธิบายสถานการณ์เดียวกัน`
    ]
  };
  return x[domain]||x.general;
}
function deckSlides(subject,unit,{admin=false}={}){
  ...
  const deep=topicExplanation(domain,topic),questions=analyticalQuestions(domain,topic,c);
  const conceptSlide=(i)=>({
    kicker:`CONCEPT ${i+1}`,
    title:c[i],
    explain:`${conceptExplanation(domain,c[i],topic)}${k[i]?` ${k[i]}`:""}`,
    body:[
      `ความหมายและหน้าที่ของ “${c[i]}” ในบริบทของ “${topic}”`,
      `ความสัมพันธ์กับ ${c.filter((_,n)=>n!==i).slice(0,2).join(" และ ")||"องค์ประกอบอื่นของหน่วย"}`,
      `เงื่อนไขหรือสถานการณ์ที่แนวคิดนี้มีผลต่อการตัดสินใจ`,
      `หลักฐานหรือผลลัพธ์ที่ใช้ยืนยันว่าเข้าใจและนำไปใช้ได้ถูกต้อง`
    ],
    example:conceptExample(domain,c[i],topic,i)
  });
  const slides=[
    {kicker:`${subject?.code||""} • UNIT ${unit?.unit_no||""}`,title:topic,lead:subject?.name||"",explain:deep,body:[`หน่วยนี้อธิบาย “${topic}” ตั้งแต่ความหมาย แนวคิดหลัก องค์ประกอบ และหลักการทำงาน`,`เนื้อหาจะค่อย ๆ เชื่อมจากพื้นฐานไปสู่ตัวอย่างและสถานการณ์จริง`,`ช่วงท้ายมีคำถามเชิงคิดวิเคราะห์เพื่อฝึกใช้เหตุผล แยกแยะข้อมูล และอธิบายการตัดสินใจ`]},
    {kicker:"MEANING & IMPORTANCE",title:"ความหมายและความสำคัญของหน่วย",explain:deep,body:(k.length?k.slice(0,4):fallback.slice(0,4)).map(x=>String(x)),example:topicScenario(domain,topic)},
    {kicker:"SCOPE & CONTEXT",title:"ขอบเขตและบริบทที่เกี่ยวข้อง",explain:`การเข้าใจ “${topic}” ต้องรู้ว่าหัวข้อนี้เกี่ยวข้องกับสิ่งใด อยู่ในขั้นตอนไหนของงาน และมีผลต่อส่วนอื่นอย่างไร เพื่อไม่ให้มองเนื้อหาแยกส่วน`,body:[`บริบทหลัก: ${subject?.name||"รายวิชา"}`,`ประเด็นที่ต้องเชื่อมโยง: ${c.slice(0,3).join(" • ")}`,`ผลกระทบเมื่อเข้าใจหรือปฏิบัติผิด: คุณภาพ ความถูกต้อง ความปลอดภัย หรือประสิทธิภาพของงานอาจลดลง`,`แนวทางศึกษา: ทำความเข้าใจเหตุผลของแต่ละองค์ประกอบก่อนจำขั้นตอน`],example:`ลองอธิบายว่า “${topic}” เชื่อมกับงานจริงของรายวิชานี้ตรงจุดใดบ้าง`},
    {kicker:"KEY CONCEPTS",title:"แนวคิดหลักของหน่วย",explain:"แนวคิดหลักทำหน้าที่เป็นโครงสร้างความคิดของหน่วย ผู้เรียนควรเข้าใจทั้งความหมายและความสัมพันธ์ระหว่างแนวคิด ไม่ใช่จำเป็นรายการ",body:c.map((x,i)=>`${i+1}. ${x}${k[i]?` — ${k[i]}`:""}`)},
    conceptSlide(0),conceptSlide(1),conceptSlide(2),conceptSlide(3),conceptSlide(4),
    {kicker:"PRINCIPLES",title:"หลักการสำคัญที่ใช้ในหน่วยนี้",explain:`หลักการช่วยอธิบายว่าเหตุใดจึงควรทำสิ่งหนึ่งก่อนอีกสิ่งหนึ่งในเรื่อง “${topic}” และช่วยให้ตัดสินใจได้เมื่อสถานการณ์ไม่เหมือนตัวอย่าง`,body:principles,example:`ให้เลือกหลักการหนึ่งข้อแล้วอธิบายผลที่จะเกิดขึ้นหากละเลยหลักการนั้นในงาน “${topic}”`},
    {kicker:"COMPONENTS & RELATIONSHIPS",title:"องค์ประกอบและความสัมพันธ์",explain:`องค์ประกอบของ “${topic}” ไม่ได้ทำงานแยกกัน การเปลี่ยนส่วนหนึ่งอาจกระทบอีกส่วน จึงต้องมองทั้ง Input, เงื่อนไข, กระบวนการ และผลลัพธ์ร่วมกัน`,body:c.map((x,i)=>`${x} → เชื่อมกับ ${c[(i+1)%c.length]} โดยพิจารณาหน้าที่ เงื่อนไข และผลกระทบต่อกัน`),example:`สร้างแผนภาพความสัมพันธ์จาก ${c.slice(0,3).join(" → ")} แล้วอธิบายว่าถ้าส่วนกลางผิดจะกระทบส่วนปลายอย่างไร`},
    {kicker:"HOW IT WORKS",title:"หลักการทำงานและกลไก",explain:`เมื่อมอง “${topic}” เป็นระบบ ควรติดตามว่าข้อมูล/เงื่อนไขเริ่มต้นผ่านการตัดสินใจและการปฏิบัติอย่างไรจนเกิดผลลัพธ์`,body:[`Input / เงื่อนไขเริ่มต้น: ระบุข้อมูลหรือทรัพยากรที่ต้องพร้อม`,`Process / กลไก: ${p.slice(0,3).join(" → ")}`,`Control / จุดตรวจ: ${(controls.length?controls.slice(0,3):["ตรวจเงื่อนไขก่อนเริ่ม","ตรวจระหว่างทำ","ตรวจผลสุดท้าย"]).join(" • ")}`,`Output / ผลลัพธ์: ต้องตรวจได้ด้วยหลักฐาน ไม่อาศัยความรู้สึกเพียงอย่างเดียว`],example:k[4]||conceptExample(domain,c[4],topic,4)},
    {kicker:"PROCESS",title:"ลำดับขั้นตอนและเหตุผลของแต่ละขั้น",explain:`ขั้นตอนของ “${topic}” ควรเรียงให้ตรวจสอบย้อนกลับได้ แต่ละขั้นต้องรู้ว่ารับอะไรเข้ามา ทำอะไร และส่งผลอะไรต่อขั้นถัดไป`,body:p.map((x,i)=>`${i+1}. ${x} — อธิบายเหตุผลของขั้นนี้ สิ่งที่ต้องตรวจ และผลที่คาดว่าจะได้`),example:`ถ้าข้ามขั้น “${p[1]||p[0]}” ให้คาดการณ์ว่าข้อผิดพลาดใดอาจตามมาและจะตรวจพบอย่างไร`},
    {kicker:"APPLICATION EXAMPLE",title:"ตัวอย่างการประยุกต์ใช้",explain:`ตัวอย่างที่ดีควรช่วยให้เห็นว่าแนวคิดของ “${topic}” ถูกนำไปใช้ตัดสินใจหรือแก้ปัญหาอย่างไรในสถานการณ์จริง`,body:[conceptExample(domain,c[0],topic,0),conceptExample(domain,c[1],topic,1),`จุดสังเกต: ต้องอธิบายเหตุผลว่าทำไมจึงเลือกแนวทางนั้น และใช้หลักฐานอะไรตรวจผล`,`การประยุกต์ใช้ที่ถูกต้องต้องปรับตามบริบท ไม่คัดลอกขั้นตอนโดยไม่ตรวจเงื่อนไข`]},
    {kicker:"CASE STUDY",title:"กรณีศึกษาเพื่อทำความเข้าใจเนื้อหา",explain:caseStudy,body:["เริ่มจากแยกข้อเท็จจริงที่ทราบแน่นอน","เชื่อมข้อเท็จจริงกับแนวคิดหลักของหน่วย","พิจารณาทางเลือกมากกว่าหนึ่งแบบก่อนตัดสินใจ","อธิบายผลที่คาดว่าจะเกิดจากแต่ละทางเลือก","กำหนดหลักฐานที่จะใช้ตรวจผลหลังดำเนินการ"],example:"กรณีศึกษาไม่ได้มีไว้จำคำตอบ แต่มีไว้ฝึกมองความสัมพันธ์ของข้อมูล เหตุผล และผลลัพธ์"},
    {kicker:"CORRECT / INCORRECT",title:"ตัวอย่างที่ถูก ตัวอย่างที่ผิด และข้อผิดพลาดที่พบบ่อย",explain:"การเปรียบเทียบสองแบบช่วยให้เห็นเส้นแบ่งระหว่างการทำตามหลักการกับการทำเพียงให้เสร็จ โดยควรอธิบายได้ว่าจุดใดทำให้ผลต่างกัน",body:[compare.good,compare.bad,...mistakes.slice(0,3).map((x,i)=>`ข้อผิดพลาดที่พบบ่อย ${i+1}: ${x}`)],example:"เมื่อพบตัวอย่างที่ผิด ให้ระบุสาเหตุก่อนเสนอวิธีแก้ เพื่อไม่แก้เพียงอาการ"},
    {kicker:"TROUBLESHOOTING & PRECAUTION",title:"การแก้ปัญหาและข้อควรระวัง",explain:`เมื่อผลของ “${topic}” ไม่เป็นไปตามคาด ควรไล่ตรวจจากหลักฐานทีละขั้น พร้อมระวังผลกระทบต่อคน ข้อมูล ระบบ หรือความปลอดภัย`,body:[...troubleshootingGuide(domain,topic),...topicPrecautions(domain,topic).slice(0,3)],example:"แก้ทีละสาเหตุ ทดสอบซ้ำด้วยเกณฑ์เดิม และบันทึกสิ่งที่เปลี่ยน เพื่อแยก Root cause ออกจากความบังเอิญ"},
    {kicker:"ANALYTICAL QUESTIONS",title:"คำถามคิดวิเคราะห์จากสถานการณ์",explain:`ใช้เนื้อหาของ “${topic}” ตอบโดยระบุเหตุผล หลักฐาน และความสัมพันธ์ของแนวคิด หลีกเลี่ยงคำตอบแบบจำชื่อหรือเลือกจากความคุ้นเคย`,body:questions.slice(0,4)},
    {kicker:"COMPARE & DISTINGUISH",title:"คำถามเปรียบเทียบและแยกแยะ",explain:"คำถามชุดนี้ฝึกแยกความเหมือน–ความต่าง หาเงื่อนไขที่ทำให้คำตอบเปลี่ยน และตัดสินใจจากหลักฐานมากกว่าความรู้สึก",body:questions.slice(4,8)},
    {kicker:"UNIT SUMMARY",title:"สรุปสาระสำคัญของหน่วย",explain:`“${topic}” ควรถูกเข้าใจเป็นระบบ ตั้งแต่ความหมาย แนวคิดสำคัญ ความสัมพันธ์ หลักการทำงาน ขั้นตอน การประยุกต์ และการตรวจผล`,body:[`แนวคิดหลัก: ${c.slice(0,5).join(" • ")}`,`หลักการสำคัญ: ${principles.slice(0,2).join(" • ")}`,`กระบวนการ: ${p.slice(0,5).join(" → ")}`,`จุดที่ต้องระวัง: ${mistakes.slice(0,2).join(" • ")}`,"ก่อนจบหน่วยควรอธิบายได้ว่าอะไรเกิดขึ้น เพราะเหตุใด เลือกวิธีอย่างไร และมีหลักฐานอะไรยืนยันผล"]}
  ];
  return slides.slice(0,20);
}
// V20.0 compatibility markers retained for historical regression only; V20.1 does NOT render these as slide pages:
// DEFINITION | KEY CONCEPTS | PRINCIPLE | PROCESS | CORRECT / INCORRECT | CASE STUDY | COMMON MISTAKE + TROUBLESHOOTING | SAFETY / PRECAUTION
// WORKSHEET • DIGITAL | WORKSHEET • PAPER | EXAM ALIGNMENT | TEACHER NOTE + REVIEW | KEY TAKEAWAY | Learning Goal | Question Bank | Answer Key | ไม่เปิดเผยข้อสอบ | ON-TIME WORK | LATE WORK
// historical wording: วิเคราะห์ปัญหาอย่างเป็นระบบ
