...
const V15_VERSION="V16.4-THAI-REG-FILTERS-PROFILE-ANALYTICS";
...
async function renderAccountApprovals(){
  ...
  const {data,error}=await client().from("profiles")
    .select("id,username,full_name,display_name,student_code,contact_email,phone,grade_level,room_label,class_name,department,major,approval_status,approval_requested_at,approved_at,rejection_reason,active,created_at")
...
async function renderStudentDashboard(){
  ...
  const p=await getProfile(),[courses,rows,summary,enrollments]=await Promise.all([approvedCourses(),studentWorkRows(),client().rpc("my_attendance_summary"),enrollmentRowsMine()]);
  ...
  <div class="v14-action-grid v161-menu-grid v161-student-menu">
    <button data-v14-route="courses"><span>🎓</span><b>ห้องเรียนของฉัน</b><small>ใบงาน สื่อ การสอบ และความคืบหน้าแยกตามรายวิชา</small></button>
    <button data-v14-route="enroll"><span>✅</span><b>ลงทะเบียนรายวิชา</b><small>ส่งคำขอเข้าเรียนและติดตามสถานะอนุมัติ</small></button>
    <button data-v14-route="work"><span>📋</span><b>สถานะงาน</b><small>ดูงานที่ยังไม่ส่ง ส่งแล้ว และใกล้ครบกำหนด</small></button>
    <button data-v14-route="attendance"><span>📷</span><b>QR / การเข้าเรียน</b><small>แสดง QR เช็คชื่อและประวัติการเข้าเรียนแบบ Real-time</small></button>
    <button data-v14-route="exam"><span>🧪</span><b>ระบบสอบ</b><small>เข้าสอบที่ได้รับมอบหมายและดูสถานะการสอบ</small></button>
    <button data-v16-base-route="profile"><span>🪪</span><b>โปรไฟล์ของฉัน</b><small>ดูข้อมูลส่วนตัวแบบอ่านอย่างเดียว ไม่สามารถแก้ไขเอง</small></button>
  </div>
...
async function renderAdminProfiles(){
  ...
  const {data,error}=await client().from("profiles").select("id,username,full_name,display_name,student_code,avatar_path,phone,grade_level,room_label,class_name,department,major,seat_number,approval_status,academic_status,active,last_seen_at,created_at").eq("role","user").order("full_name");
  ...
  overlay(`<div class="v14-modal-head"><div><span class="v14-kicker">ADMIN STUDENT PROFILE</span><h2>${esc(p.full_name||"-")} ${p.display_name&&p.display_name!==p.full_name?`<small>(${esc(p.display_name)})</small>`:""}</h2><p>${esc(p.student_code||"")} • ${esc(p.class_name||"")}</p></div><button class="btn" data-v14-close>✕</button></div>
    ...
    <div class="v14-profile-detail"><div>${avatar?`<img src="${avatar}" alt="รูปโปรไฟล์">`:`<div class="v14-big-avatar">${esc((p.full_name||"?")[0])}</div>`}</div>
      <div class="v14-info-grid">
        <div><span>ชื่อ-นามสกุล</span><b>${esc(p.full_name||"-")}</b></div><div><span>ชื่อเล่น</span><b>${esc(p.display_name&&p.display_name!==p.full_name?p.display_name:"-")}</b></div>
    ...
    <h3>รายวิชาที่เกี่ยวข้อง</h3><div class="v14-chip-list">${(er.data||[]).map(x=>`<span>${esc(x.subjects?.code||"")} ${esc(x.subjects?.name||"")} • ${esc(x.status)}</span>`).join("")||"-"}</div>
...
// ---------------------------------------------------------------------------
async function boot(){
  document.documentElement.classList.add("v14-tech");document.documentElement.dataset.docnrVersion="v16.1";
...
window.DOCNR_V16_4=Object.freeze({navigate,version:V15_VERSION});
...
boot().catch(e=>console.error("DOC-FULL-NR V16.4 boot",e));
