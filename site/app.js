...
function createUserDialog(){
  modal(`<div class="modal-header"><div><h3>สร้างผู้ใช้</h3><div class="muted">บัญชีนักศึกษาใช้เลขนักศึกษาเป็นชื่อเข้าสู่ระบบ • ข้อมูลนักศึกษาต้องครบก่อนสร้างบัญชี</div></div><button class="btn sm" data-close>✕</button></div>
    <form id="cu"><div class="form-grid">
      ...
      <div class="field cu-student"><label>สาขาวิชา</label><select name="major" required>${optionHtml(REG_MAJORS,"เลือกสาขาวิชา")}</select></div>
  ...
  role.onchange=toggle;toggle();thaiOnlyInput(form.elements.full_name);thaiOnlyInput(form.elements.display_name);
...
function editUserDialog(user){
  ...
  modal(`<div class="modal-header"><div><h3>แก้ข้อมูลผู้ใช้</h3><div class="muted">${esc(user.full_name||user.username||"")} • ${isUser?"นักศึกษา":"Admin"}</div></div><button class="btn sm" data-close>✕</button></div>
    <form id="eu"><div class="form-grid">
      ...
      <div class="field"><label>สาขาวิชา</label><select name="major" required>${REG_MAJORS.map(x=>`<option value="${esc(x)}" ${user.major===x?"selected":""}>${esc(x)}</option>`).join("")}</select></div>
  ...
  const form=$("#eu");thaiOnlyInput(form.elements.full_name);if(isUser)thaiOnlyInput(form.elements.display_name);
...
async function gradeDialog(id){
  ...
  $("#gf").onsubmit=async e=>{
    e.preventDefault();const f=Object.fromEntries(new FormData(e.target));
    const r=await sb.rpc("admin_grade_submission_v177",{p_submission_id:id,p_score:Number(f.score),p_max_score:Number(f.max_score),p_grade:f.grade||null,p_admin_comment:f.admin_comment||null,p_rubric_result:{}});
    if(r.error)return toast(friendlyError(r.error),"error");
    ...
    <div class="grid two">
      <div class="card"><h3>🩺 Backend Contract</h3><div class="alert ${h?.backend_ok?"success":"error"}">${h?.backend_ok?"Backend พร้อมใช้งาน":"พบจุดที่ต้องตรวจ"}</div>
        <div class="profile-data-grid">
          ...
          <div><span>แจ้งเตือน Deadline</span><b>${h?.deadline_notification_backend_ok?"พร้อม":"ตรวจสอบ"}</b></div><div><span>Profile Read-only</span>${pass(h?.student_profile_readonly)}</div>
